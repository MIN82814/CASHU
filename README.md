...
- **首頁設置**：`pages/index.html` 為預設首頁入口，請勿隨意修改其檔案名稱以防路徑解析出錯。
- **Base 路徑配置**：專案設定中的 `base` 路徑已指定為 `/CASHU/`，用以確保在生產環境（GitHub Pages）部署時，所有靜態資產與 EJS 模板的相對路徑能被正確引用。
- **不預期樣式覆蓋**：本專案完全由 Vite 自動監聽並實時編譯產出樣式，開發時請直接於 `assets/scss/` 中擴充模塊，切勿在本地啟用傳統的 Live Sass Compiler 套件，以避免產生編譯衝突。
