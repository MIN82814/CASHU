...
> 專案核心聚焦於極致的行動端畫素級（Pixel-Perfect）精密還原、高度解耦的樣式架構，以及高度自動化的前端開發與部署工作流。
...
| **Min** | 搭建全站 SCSS 模塊化架構
封裝 `EJS` 共用模板
建置 `_navbar-bottom` 與 `_mobile-frame` 等全站核心視覺組件 | **獨立負責完整互動與頁面切換：**<br>· `log-add.html` (日誌新增)<br>· `select_account.html` (帳戶選擇)<br>· `category-page.html` (消費分類)<br>· `add-result.html` (新增結果) | **獨立負責完整互動與頁面切換：**<br>· `plan-save.html` (計畫主頁)<br>· `plan-save-detail.html` (詳情追蹤)<br>· `plan-save-success.html` (成功回饋) |
...
| 技術 / 工具 | 說明 |
| :--- | :--- |
HTML｜SCSS｜Bootstrap 5｜Git / GitHub（Gitflow、PR協作）｜Figma｜SVG
---
...
- **共用樣式**： SCSS 進行架構分層（包含 base, layout, components, pages, utils），確保樣式碼的極高可讀性與擴充性。
